...
| ID | 优先级 | 身份 | 问题 | 根因 | 变更 | 验证 | 状态 |
|----|--------|------|------|------|------|------|------|
| PATCH-016 | P1 | 外部审查者(GodBlf RFC)+独立AI | audit pipeline 只有 MCP 入口（mcp_server.py）与旧式 audit.py CLI（文本输出+退出码只有 0/1/2），shell/CI 流水线无法以机器可读、语义诚实的方式接入；GodBlf RFC 三问（exit states/audit log/unknown candidate set）无落地 | 入口适配层缺位 | 新增 `cli_adapter.py`（与 mcp_server.py 平级，stdin/文件 → 不变 envelope）：①**边界1 exit states**——verdict 与 degraded 保持分离字段；退出码按 `protocols/audit-protocol.md` **v1.1 两轴规格**落地（执行状态与 envelope 分离）：0=PASS·degraded=false｜10=PASS·degraded=true｜11=PASS-WITH-CAVEAT｜12=FAIL·degraded=false｜13=FAIL·degraded=true｜2=EXPLICIT_FAILURE（**保留号，当前无路径产出**）｜3=SILENT_TIMEOUT（无 envelope：崩溃/启动失败，绝不重标 DEGRADED）｜4=USAGE_ERROR（输入畸形，或 envelope 无 verdict——不再谎报为 FAIL）｜5=POLICY_DENIED（opt-out 不可记录 → fail closed，**与崩溃 3 区分**，根因事实不同）；②**边界2 audit log**——append-only JSONL keyed by trace_id（audit_started/verification_opt_out/audit_policy_denied/audit_finished/audit_execution_error），stdout 只走不变 envelope、诊断走 stderr，opt-out 记录失败 fail-closed **已定案**（拒绝产出无法追溯的 envelope，并独立记为 POLICY_DENIED 而非崩溃）；③**边界3 unknown candidate set**——adapter 零新语义：coverage/candidate_set 原样透传引擎，不用观测集合大小冒充未知总数，unknown total 的 coverage 表示留给 core-contract issue；取源策略显式收紧（与 mcp_server 同一 env var 显式开启机制）；**参数解析严格化**——未知选项/带值选项缺值一律 exit 4（USAGE_ERROR）（`--cache --offline` 吞 flag 会致意外在线验证、`--audit-log` 缺值静默无日志会破坏审计链，均不许静默回退）；顺带修引擎既有缺陷：`judge_claims` 对非 list `claims` 抛 TypeError 违反 audit() never-raises 契约 → 防御性返回空 | `tests/test_cli_adapter.py` **23 用例**（两轴退出码 0/10/11/12/13/2/3/4/5 全覆盖、PASS-WITH-CAVEAT→11、degraded FAIL→13 与 clean FAIL→12 区分、保留号 2 无路径产出、crash(3)≠policy-denied(5)、JSONL 追加+trace 贯穿、opt-out 事件有无两向、fail-closed mock 化、unknown total 透传不变、envelope 键集不变、私网 URL 默认拒、未知选项/缺值/吞 flag 三回归）+ `tests/test_audit.py` 2 用例（非 list claims 不崩、非 dict claim 项跳过）✅；全量 **74/74**（verify 26 + MCP 8 + adapter 5 + audit 12 + CLI adapter 23）；独立 AI 对抗验证双轮（AI#1 实测退出码+1MB 边界+内网 6 类全拒+envelope 逐字节等同原生，PASS 带 2 P1 → 全修复 → AI#2 复审：代码达标，指出 PR 草稿数字失实（已更正）与退出码表缺 degraded-无-verdict 子情形（v1.1 改判：无 verdict 归 USAGE_ERROR 4，不再有该歧义）） | open（PR 分支待推，退出码已对齐协议 v1.1） |
